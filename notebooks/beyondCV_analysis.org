...
#+BEGIN_SRC ipython :session bcv :results raw drawer
  ...
  for survey in mcmc_survey:
      path = output_path + r"/H0/results_{}_667_2/mcmc".format(survey)
      samples += [get_mean_diffs(path, range(0,6))]

  g = plots.getSubplotPlotter()
  g.settings.lineM = ["--gray"] + ["-C{}".format(i) for i in range(len(mcmc_survey))]
...
:results:
# Out[26]:
: <matplotlib.legend.Legend at 0x7f1a3aa32048>
[[file:./obipy-resources/uIWWxm.png]]
:end:
...
:results:
# Out[28]:
[[file:./obipy-resources/bQVzse.png]]
:end:
