...
#+BEGIN_SRC ipython :session bcv :results none
  ...
  def plot_chains(path=r"/home/garrido/Workdir/CMB/analysis/beyondCV/output/mcmc", pattern=None, burnin=0):
      chains = get_chains(path=path, pattern=pattern)
      ...
      for j in range(2,9):
          ...
          if j != 8:
              plt.axhline(ref[j-2], color="gray", ls="--", lw=1)
...
:results:
# Out[279]:
[[file:./obipy-resources/Fn1hDi.png]]
:end:
...
:results:
# Out[23]:
[[file:./obipy-resources/f34yVP.png]]
:end:
** ALens
#+BEGIN_SRC ipython :session bcv :results none
  labels = [r"$\theta_\mathrm{MC}$",
...
#+BEGIN_SRC ipython :session bcv :results raw output
  import os, glob
  all_files = glob.glob(os.path.join(result_path, "results*/*.pkl"))
  all_files.sort()
  table = []
  for f in all_files:
    import pickle
    msg = pickle.load(open(f, "rb"))["results"]["OptimizeResult"]["message"]
    table += [[f.split("/")[-2], msg]]
  print(tabulate.tabulate(table, headers=["Directory", "Status"], tablefmt="orgtbl"))
#+END_SRC

#+RESULTS:
| Directory                | Status                                                        |
|--------------------------+---------------------------------------------------------------|
| results_666_0_PxP        | Optimization terminated successfully.                         |
| results_666_0_SOxP-PxP   | Optimization failed. Estimated distance to minimum too large. |
| results_666_0_SOxP-SOxSO | Optimization failed. Estimated distance to minimum too large. |
| results_666_0_SOxSO-PxP  | Optimization terminated successfully.                         |
| results_666_1_PxP        | Optimization terminated successfully.                         |
| results_666_1_SOxP-PxP   | Optimization terminated successfully.                         |
| results_666_1_SOxP-SOxSO | Optimization failed. Estimated distance to minimum too large. |
| results_666_1_SOxSO-PxP  | Optimization terminated successfully.                         |
| results_666_2_PxP        | Optimization failed. Estimated distance to minimum too large. |
| results_666_2_SOxP-PxP   | Optimization failed. Estimated distance to minimum too large. |
| results_666_2_SOxP-SOxSO | Optimization failed. Estimated distance to minimum too large. |
| results_666_2_SOxSO-PxP  | Optimization terminated successfully.                         |
| results_666_3_PxP        | Optimization terminated successfully.                         |
| results_666_3_SOxP-PxP   | Optimization terminated successfully.                         |
| results_666_3_SOxP-SOxSO | Optimization terminated successfully.                         |
| results_666_3_SOxSO-PxP  | Optimization terminated successfully.                         |
| results_666_4_PxP        | Optimization failed. Estimated distance to minimum too large. |
| results_666_4_SOxP-PxP   | Optimization terminated successfully.                         |
| results_666_4_SOxP-SOxSO | Optimization failed. Estimated distance to minimum too large. |
| results_666_4_SOxSO-PxP  | Optimization terminated successfully.                         |

*** Check chains
**** PxP
#+BEGIN_SRC ipython :session bcv :results raw drawer
  plot_chains(path=result_path, pattern="results*_PxP*/mcmc*.txt", burnin=0)
#+END_SRC

#+RESULTS:
:results:
# Out[25]:
[[file:./obipy-resources/S93v14.png]]
:end:

**** SOxSO - PxP
#+BEGIN_SRC ipython :session bcv :results raw drawer
  plot_chains(path=result_path, pattern="results_*SOxSO-PxP*/mcmc*.txt", burnin=0)
#+END_SRC

#+RESULTS:
:results:
# Out[27]:
[[file:./obipy-resources/b21StY.png]]
:end:

**** SOxP - PxP
#+BEGIN_SRC ipython :session bcv :results raw drawer
  plot_chains(path=result_path, pattern="results_*SOxP-PxP*/mcmc*.txt", burnin=0)
#+END_SRC

#+RESULTS:
:results:
# Out[28]:
[[file:./obipy-resources/Xnxcs0.png]]
:end:

**** SOxP - SOxSO
#+BEGIN_SRC ipython :session bcv :results raw drawer
  plot_chains(path=result_path, pattern="results_*SOxP-SOxSO*/mcmc*.txt", burnin=0)
#+END_SRC

#+RESULTS:
:results:
# Out[29]:
[[file:./obipy-resources/Dwa4Vu.png]]
:end:
*** Triangle plot
#+BEGIN_SRC ipython :session bcv :results none
  def plot_samples(path=result_path, name=None, burnin=0.3):
      samples = []
      for i in range(5):
          pattern = "results*_{}_{}*/mcmc*.txt".format(i, name)
          all_files = glob.glob(os.path.join(result_path, pattern))
          if not all_files:
              continue
          s, w = get_samples(result_path, pattern)
          samples += [MCSamples(samples=s, names=labels, label="{} - MCMC {}".format(name, i), weights=w, ignore_rows=burnin)]
...
**** PxP
#+BEGIN_SRC ipython :session bcv :results raw drawer
  plot_samples(name="PxP")
#+END_SRC

#+RESULTS:
:results:
# Out[37]:
[[file:./obipy-resources/c0n3GN.png]]
:end:
**** SOxSO - PxP
#+BEGIN_SRC ipython :session bcv :results raw drawer
  plot_samples(name="SOxSO-PxP")
#+END_SRC

#+RESULTS:
:results:
# Out[38]:
[[file:./obipy-resources/TGgDOh.png]]
:end:
